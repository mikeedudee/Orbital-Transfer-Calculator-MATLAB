function [radius_initial, radius_final, velocity_initial_orbit, velocity_final_orbit, ...
    ...
    deltaV_1, deltaV_2, deltaV_total, transfer_time, efficiency] = hohmann_orbital_parameters(initial_altitude, final_altitude, inclination_c, efficiency)
    ...
    eccentricity = (radius_final - radius_initial) / (radius_final + radius_initial);
        if eccentricity < 0
            eccentricity = 0; % Ensure non-negative eccentricity
        end

    % Calculate the Periapsis and Apoapsis Velocities
    inclination        = inclination_c;
    periapsis_velocity = sqrt(cvc.Earth_gravitational_constant/radius_initial) * sqrt(1 + (eccentricity * cos(inclination_c)));
    apoapsis_velocity  = sqrt(cvc.Earth_gravitational_constant/radius_final) * sqrt(1 - (eccentricity * cos(inclination_c)));
