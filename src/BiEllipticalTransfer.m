...
fprintf('Intermediate altitude computation options:\n');
disp(['[' 8 '<1> Optimal]' 8 ''])
disp('<2> User-defined')
iac_type = get_selection_input(['\nSelection (1 or 2): '], [1, 2]);

    if iac_type == 2
        disp(['Enter the bi-elliptic altitude [' 8 '(km)]' 8 ':'])
        altb = get_positive_input('> ');
    end
    ...
    if (iac_type == 1)
        ...
        rb = altb + req; % user-defined intermediate altitude
    end
...
function print_results(alti, altf, altb, vi, vt1a, vt1b, vt2b, vt2c, vf, ecct1, ecct2, dva, dvb, dvc, tof1, tof2, tof)
    fprintf('<strong>Bi-Elliptic Orbit Transfer Analysis Results\n</strong>');
    fprintf('--------------------------------------------\n');
    fprintf('Initial orbit altitude:      %14.4f km\n', alti);
    fprintf('Final orbit altitude:        %14.4f km\n', altf);
    fprintf('Intermediate altitude:       %14.4f km\n', altb);
    fprintf('Initial velocity:            %14.4f m/s\n', vi*1000);
    fprintf('First ellipse velocities:    %14.4f and %14.4f m/s\n', vt1a*1000, vt1b*1000);
    fprintf('Second ellipse velocities:   %14.4f and %14.4f m/s\n', vt2b*1000, vt2c*1000);
    fprintf('Final velocity:              %14.4f m/s\n', vf*1000);
    fprintf('Eccentricity (1st ellipse):  %14.8f\n', ecct1);
    fprintf('Eccentricity (2nd ellipse):  %14.8f\n', ecct2);
    fprintf('Delta-v:                     %14.4f, %14.4f, %14.4f m/s\n', dva*1000, dvb*1000, dvc*1000);
    fprintf('Total delta-v:               %14.4f m/s\n', (dva+dvb+dvc)*1000);
    fprintf('Transfer times:              %14.4f, %14.4f, %14.4f hours\n', tof1/3600, tof2/3600, tof/3600);
end
